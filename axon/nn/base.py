...
import axon as ax
from axon.utils import tree_flatten
...
class Module(dict):
    ...
    def __setattr__(self, key: str, val: Any):
        if isinstance(val, (ax.Tensor, dict, list, tuple)):
    ...
    # def load_weights(
    #     self,
    #     file_or_weights: Union[str, List[Tuple[str, ax.Tensor]]],
    #     strict: bool = True,
    # ) -> "Module":
    #     """
    ...
    def valid_parameter_filter(module, key, value):
        return isinstance(value, (dict, list, ax.Tensor)) and not key.startswith("_")
    ...
    def filter_and_map(
        self,
        filter_fn: Callable[["ax.nn.Module", str, Any], bool],
        map_fn: Optional[Callable] = None,
        is_leaf_fn: Optional[Callable[["ax.nn.Module", str, Any], bool]] = None,
    ):
            ...
            if isinstance(parameters, dict):
                for k in parameters:
                    if k in dst:
                        ...
                        if isinstance(current_value, ax.Tensor):
                            dst[k] = new_value
    ...
    def apply(
        self,
        map_fn: Callable[[ax.Tensor], ax.Tensor],
        filter_fn: Optional[Callable[["ax.nn.Module", str, Any], bool]] = None,
    ) -> "Module":
    ...
    def apply_to_modules(
        self, apply_fn: Callable[[str, "ax.nn.Module"], Any]
    ) -> "Module":
...
def value_and_grad(model: Module, fn: Callable):
    ...
    value_grad_fn = ax.value_and_grad(inner_fn)
